...
public:
    ...
    class spell_q12641_death_comes_from_on_high_summon_ghouls_SpellScript : public SpellScript
    {
        ...
        {
            OnEffectHitTarget.Register(&spell_q12641_death_comes_from_on_high_summon_ghouls_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);            
        }
    };
...
public:
    ...
    struct npc_death_knight_initiateAI : public CombatAI
    {
        ...
        void Reset() override
        {
            ...
            me->SetUInt32Value(UNIT_NPC_FLAGS, NPCFlags::UNIT_NPC_FLAG_GOSSIP);
        }

        void SpellHit(WorldObject* caster, SpellInfo const* pSpell) override
        {
            if (Unit* casterUnit = caster->ToUnit())
            {
                if (!_duelInProgress && pSpell->Id == SPELL_DUEL)
                {
                    ...
                    me->GetMotionMaster()->MoveFollow(casterUnit, 2.0f, 0.0f);
...
public:
    ...
    class spell_q12698_the_gift_that_keeps_on_giving_SpellScript : public SpellScript
    {
        void HandleScriptEffect(SpellEffIndex /*effIndex*/)
        {
            if (Unit* hu = GetHitUnit())
            {
                if (Unit* oc = GetOriginalCaster())
                {
                    uint32 summonSpellId = GetEffectValue();
                    uint32 ghostRate = urand(0, 100);
                    if (ghostRate < 50)
                    {
                        summonSpellId = SPELL_SUMMON_SCARLET_GHOST;
                    }
                    oc->CastSpell(hu, summonSpellId, true);
                }
            }
        }
        ...
        void Register() override
        {
            OnEffectHitTarget.Register(&spell_q12698_the_gift_that_keeps_on_giving_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    struct npc_scarlet_ghoulAI : public ScriptedAI
    {
        ...
        void SetGUID(const ObjectGuid& guid, int32) override
        {
            gothikGUID = guid;
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_GHOUL_RESTORE_STATE:
                    ...
                    if (Player* owner = me->GetCharmerOrOwnerPlayerOrPlayerItself())
                        me->GetMotionMaster()->MoveFollow(owner, DEFAULT_FOLLOW_DISTANCE_PET, frand(0.0f, 2 * M_PI));
    ...
    struct npc_scarlet_miner_cartAI : public PassiveAI
    {
        ...
        void DoAction(int32 /*param*/) override
        {
            if (Creature* miner = ObjectAccessor::GetCreature(*me, minerGUID))
            {
                ...
                me->SetSpeed(UnitMoveType::MOVE_RUN, 10.0f);
                me->SetSpeed(UnitMoveType::MOVE_WALK, 10.0f);
                me->SetWalk(false);
                me->GetMotionMaster()->MoveFollow(miner, 1.0f, 0.0f, false, false, false, MovementSlot::MOTION_SLOT_ACTIVE);
    ...
    struct npc_scarlet_minerAI : public EscortAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (startDelay > 0)
            {
                ...
                {
                    startDelay = 0;
                    if (Player* targetPlayer = ObjectAccessor::FindPlayerByLowGUID(playerGUIDLow))
                    {
                        if (Creature* car = targetPlayer->GetVehicleCreatureBase())
                        {
                            carGUID = car->GetGUID();
                            car->AI()->SetGUID(me->GetGUID());                            
                            InitCartQuest(targetPlayer);
                        }
                    }
                }
            }
...
public:
    ...
    struct go_inconspicuous_mine_carAI : public GameObjectAI
    {
        go_inconspicuous_mine_carAI(GameObject* go) : GameObjectAI(go) { }
        ...
        bool OnGossipHello(Player* player, GameObject* /*go*/) override
        {
            if (player->GetQuestStatus(12701) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                if (Creature* miner = player->SummonCreature(28841, 2383.869629f, -5900.312500f, 107.996086f, player->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, 1))
                {
                    ...
                    miner->AI()->SetData(10, player->GetGUID().GetCounter());
                }
            }
    ...
    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_inconspicuous_mine_carAI(go);
    }
};
...
class spell_death_knight_initiate_visual : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHitTarget.Register(&spell_death_knight_initiate_visual::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
